...
/**
 * お試しの入口の文言（C13・2026-09-11／文言と行き先は 2026-09-12 に変更）。
 * **LP の主役で主ボタンの隣に並ぶ。**
 *
 * `CTA_PRIMARY_LABEL` と違って価格を名乗っていないので `SITE_IS_FREE` には連動しない。
 * ここに置いてあるのは、主ボタンと**対で読まれる1組**だからで、
 * 片方だけ別の場所にあると言い回しが割れる。
 *
 * **括弧の中に「会員登録なし」と書いてある**のが要点（オーナー判断 2026-09-12）。
 * 押す前にいちばん知りたいのは「登録が要るのか」なので、それをボタンの中で答える。
 *
 * 行き先は**ステージ選択（`/stages`）**。問題へ直行させない ── 先に全体像
 * （何問あって、いまどこが開いているか）を見せてから中に入ってもらう。
 */
export const CTA_PREVIEW_LABEL = "お試しでやってみる（会員登録なし）";

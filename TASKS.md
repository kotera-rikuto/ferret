...
- [x] ステージ選択画面（`/stages`）— ジグザグマップ・クリア状態表示 ※**モックデータ、DB未接続**
- [x] 問題画面（`/problems/[id]`）— コード表示 + 回答入力 + 採点API呼び出し ※**モックデータ、Shiki未使用（`<pre>` 直書き）**。params は修正済み
- [x] リザルト画面（`/result/[id]`）— スコア + 合否 + フィードバック ※**DB未接続。フィードバックはスコア閾値によるハードコード文言で、AI採点結果を未使用。XPバー未実装**。searchParams は修正済み
...
- [ ] **一度も実際に動かしていない** — `OPENAI_API_KEY` が空のため未検証。キー設定後に疎通確認する

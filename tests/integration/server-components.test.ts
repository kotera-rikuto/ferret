...
describe("§10-2 /problems/[id]", () => {
  ...
  it("I-376 表示に必要な列だけを取り、模範解答を読まない", async () => {
    ...
    // scenario（場面）・context（実行結果）・prerequisite（前提知識）は
    // 入っている問題だけ出す（app/problems/[id]/page.tsx）
  ...
  it("I-376c 場面・実行結果・前提知識が空でも描画できる", async () => {
    // 大多数の問題は3欄とも NULL。ここで落ちると、欄を足した瞬間に
    // 全問が 404 になる（PROBLEM_DETAIL は3欄を持たないので undefined で入る）
    expect(await outcome(() => ProblemPage(params(String(UNLOCKED_ID))))).toBe(
      "RENDERED",
    );
  });

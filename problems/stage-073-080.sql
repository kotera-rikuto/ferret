...
console.log(toYen([1, 2, 3]));',
  ...
  '2つ目で呼ばれるのは、format.js の toYen ではなく default に指定された summarize です。
...
そのため toYen([1, 2, 3]) は summarize を呼び、出力は 0.1 と 3件 になります。format.js の toYen は読み込まれていないので使われません。一方 TAX_RATE は中括弧の中なので、export した側と同じ名前でなければ受け取れません。',
  '[{"match":["toYen","default"]},{"match":["summarize","3件"]},{"match":["中括弧の外","自由に決め","何でもよ"]},{"match":["TAX_RATE","中括弧の中","同じ名前"]}]'::jsonb,
  '{"core":"toYen という名前で入ってくるのが default の summarize だという結論を指していれば満たす","depth":"出力が 3件 になり format.js の toYen が使われない点、または TAX_RATE は同じ名前でなければ受け取れない点に触れていれば満たす","ground":"中括弧の外に書いた名前は default に結び付き読み込む側が自由に決められる点に触れていれば満たす","core_reject":["2つ目が format.js の toYen を呼ぶと読んでいる","名前が違うので実行時エラーになると読んでいる","TAX_RATE も自由な名前で受け取れると読んでいる"]}'::jsonb,
  'export には2種類あります。export const x = … のように名前を付けて出すものと、export default … で1つだけ出せるものです。
...
ESM の取り込みは実行が始まる前に決まるので、これは実行時のエラーではなく読み込み時のエラーになります。try で囲んでも捕まりません。default として1つ受け取ってから、そこから中身を取り出す形にすれば動きます。',
  '[{"match":["VERSION","名前を指定"]},{"match":["失敗","受け取る先","できず"]},{"match":["module.exports","名前ごと","1つ渡す"]},{"match":["default","実行が始まる前","読み込み時"]}]'::jsonb,
  '{"core":"名前を指定した取り込みができず読み込みの時点で失敗するという結論を指していれば満たす","depth":"実行が始まる前に決まるので try で囲んでも捕まらない点、または default として受け取れば動く点に触れていれば満たす","ground":"CommonJS 側が1つのオブジェクトを渡しているだけで名前ごとに出していない点に触れていれば満たす","core_reject":["VERSION に 1.4.0 が入ると読んでいる","実行時に undefined になるだけだと読んでいる","CommonJS と ESM はどう書いても混ぜられないと読んでいる"]}'::jsonb,
  'CommonJS では module.exports = 値 で1つの値を外へ出します。ESM では export を使って、出すものに名前を付けられます。
...
もう2つ読み取れます。開発のときだけ使う側に並んでいる vitest と typescript は、公開用の導入では外せます。ci は && でつないでいるので、build が失敗すると test は走らずにそこで止まります。',
  '[{"match":["3.22.4","2.30.0"]},{"match":["上がってよい","上がる","新しくな"]},{"match":["5.4.5","固定","動かな"]},{"match":["devDependencies","&&","build が失敗"]}]'::jsonb,
  '{"core":"3つのバージョン指定で上がってよい幅が違うという結論を指していれば満たす","depth":"devDependencies が公開用の導入では外せる点、または ci が && なので build の失敗で test が走らない点に触れていれば満たす","ground":"^ と ~ と記号なしで新しくなる範囲が変わる点に触れていれば満たす","core_reject":["^ と ~ が同じ意味だと読んでいる","typescript もほかと同じように上がると読んでいる","devDependencies も公開用の導入に含まれると読んでいる"]}'::jsonb,
  'dependencies はアプリを動かすのに要るもの、devDependencies は開発のときだけ要るものです。公開用に入れるときは後者を外せます。
...
export async function convert(inputPath, outDir) {
  ...
  '場所の組み立てを自分の手でやらず、環境や入力の形の違いを吸収させるためです。
...
basename と extname を通しているのも同じ考えです。入力が相対でも絶対でも、どれだけ深い場所にあっても、拡張子が何であっても、取り出せる名前は同じになります。区切りの記号は OS によって違うので、そこも任せています。',
  '[{"match":["outPath","outDir"]},{"match":["吸収","気にしなくて","任せ"]},{"match":["末尾","付いているかどうか","結果が変わ"]},{"match":["相対","絶対","拡張子","OS"]}]'::jsonb,
  '{"core":"場所の組み立てを自分でやらず環境や入力の形の違いを吸収させるという意図を指していれば満たす","depth":"入力が相対でも絶対でも同じ名前が取れる点、または区切りの記号が OS によって違う点に触れていれば満たす","ground":"文字列の足し算だと outDir の末尾の有無で結果が変わる点に触れていれば満たす","core_reject":["処理を速くするために使っていると読んでいる","入力のファイルを書き換えるために使っていると読んでいる","出力先を outDir ではなく入力と同じ場所にするためだと読んでいる"]}'::jsonb,
  'node:path の join(…) は、渡した断片をその環境の区切り記号でつなぎます。basename(パス, 取り除きたい部分) はいちばん後ろの名前だけを取り出し、extname(パス) は . から後ろを返します。
...
console.log(resolveConfig({ NODE_ENV: "development" }));',
  ...
  '1つ目の設定では retries が 3 ではなく 0 になります。
...
環境変数の値はすべて文字として届くので、DEBUG も "false" という中身のあるものです。=== "true" と比べているので false になります。2つ目の呼び出しでは NODE_ENV が production ではないので apiBase は localhost のほうになり、RETRIES が無いので retries は 3 になります。',
  '[{"match":["retries","1つ目の設定"]},{"match":["3 ではな","0 が残"]},{"match":["どちらでもな","そのまま残","左が"]},{"match":["DEBUG","false","localhost","3 になり"]}]'::jsonb,
  '{"core":"1つ目で retries が 3 ではなく 0 になるという結論を指していれば満たす","depth":"DEBUG が \"false\" という中身のある値でも === \"true\" で false になる点、または2つ目では RETRIES が無いので 3 になる点に触れていれば満たす","ground":"?? が左を使わないのは null か undefined のときだけである点に触れていれば満たす","core_reject":["1つ目の retries が 3 になると読んでいる","1つ目の debug が true になると読んでいる","2つ目の apiBase が https のほうになると読んでいる"]}'::jsonb,
  '環境変数の値は、数字を書いても真偽を書いても、受け取るときはすべて文字列です。"0" も "false" も中身のある文字列です。
...
describe("toSlug", () => {
  ...
  ' ✓ lib/slug.test.js > toSlug > 空白をつなぐ
 ...
 Test Files  1 failed (1)
  ...
  '原因は、記号をハイフンに変える行と、空白をハイフンに変える行が続けざまに効いていることです。
...
ほかの3件は通っているので、原因はカンマのような記号を含む入力に限られると分かります。trim の行も、空白をまとめる行も、それぞれ別の1件が通っているので無関係です。',
  '[{"match":["カンマ","記号をハイフン"]},{"match":["2つ","--","重なっ"]},{"match":["空白をハイフン","続けざま","そのあと"]},{"match":["3件","trim","通って"]}]'::jsonb,
  '{"core":"記号をハイフンに変える行と空白をハイフンに変える行が続けざまに効いているのが原因だという結論を指していれば満たす","depth":"ほかの3件が通っているのでカンマを含む入力だけで起きると絞れる点、または trim や空白をまとめる処理は無関係だと分かる点に触れていれば満たす","ground":"実行結果の expected ''hello--world'' to be ''hello-world'' がハイフン1つぶんの差である点に触れていれば満たす","core_reject":["trim が効いていないのが原因だと読んでいる","空白をまとめる処理が原因だと読んでいる","期待した値のほうが間違っていると読んでいる"]}'::jsonb,
  'describe と it はテストのまとまりと1件を表します。expect(実際).toBe(期待) は、左右が同じかを確かめます。
...
values (
  ...
  5,
  'ズレ',
  '// このファイルの関数はすべて「渡された配列を変えずに新しい配列を返す」ことになっている
...
console.log(src);',
  ...
  '決まりを守っていないのは sortedByLength だけです。
...
ほかの3つは決まりどおりです。head の slice も withoutEmpty の filter も別の配列を返しますし、appended はスプレッドで組み直しているので、渡された配列に手を入れていません。',
  '[{"match":["sortedByLength","sort"]},{"match":["src","並べ替わ","変えて"]},{"match":["新しい配列を作らな","その場で","同じ配列"]},{"match":["slice","filter","決まりどおり","2行"]}]'::jsonb,
  '{"core":"sortedByLength だけが渡された配列を変えてしまうという結論を指していれば満たす","depth":"出力の2行がどちらも同じ中身になる点、または残る3つは別の配列を返すので決まりどおりである点に触れていれば満たす","ground":"sort が新しい配列を作らずその場で並べ替えて同じ配列を返す点に触れていれば満たす","core_reject":["appended が渡された配列を変えると読んでいる","4つとも決まりどおりだと読んでいる","withoutEmpty が渡された配列から空文字を消すと読んでいる"]}'::jsonb,
  '配列のメソッドには、呼ばれた配列そのものに手を入れるものと、手を入れずに別の配列を返すものがあります。ステージ30 で扱った区別です。

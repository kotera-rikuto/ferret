// ステージ 60〜72 の問題データ（第9章 時間差のあるコードを読む）。**構成案で最大の章。**
//
// 螺旋の回収が2本ぶら下がる。
//   72 ← 46（JSON の往復）・57（try の範囲）… 回収元は両方とも投入済み
//   65 → 97（Promise<T> の型）              … こちらが回収元になる
//
// 投入前に必ず次の2つを回すこと。
//   node problems/preflight.mjs problems/stage-060-072.data.mjs
//   node problems/run-code.mjs  problems/stage-060-072.data.mjs
//
// ⚠️ 非同期の問題なので、run-code.mjs は setTimeout の注入・待ち合わせ・
//    未処理の拒否の受け止めに対応した版であること（2026-08-19 に更新済み）。

export const problems = [
  {
    order: 60,
    title: "同期と非同期 ─ コールバックが「あとで」呼ばれる",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
setTimeout(() => console.log("2回目", getTheme()), 10);`,
    ...
    model_answer: `最初の行は「1回目 未取得」になります。
...
2回目は dark になります。1回目の呼び出しで予約された処理がそのあいだに走り、cachedTheme を書き換えているためです。getTheme 自体は待つ仕組みを持っていないので、いつも「そのときに入っている値」を返しているだけです。`,
    prerequisite: `setTimeout(関数, ミリ秒) は、その関数をあとで実行するように予約するだけで、その場では走りません。予約した時点で次の行へ進みます。

0 を指定しても「すぐ」ではなく、いま動いている処理がすべて終わってからになります。

関数を引数として渡し、あとから呼んでもらう書き方では、呼ばれる時点が渡した側の実行より後になります。`,
    keywords: [
      { match: ["1回目", "getTheme", "cachedTheme"] },
      { match: ["未取得", "間に合いません", "まだ入って"] },
      { match: ["return のあと", "先に return", "あとから呼ば"] },
      { match: ["2回目", "dark", "待つ"] },
    ],
    rubric_items: {
      core: "最初の行が dark ではなく 未取得 になるという結論を指していれば満たす",
      ground:
        "渡した関数が呼ばれるのは return のあとなので代入が間に合わない点に触れていれば満たす",
      depth:
        "2回目は前の呼び出しで入った dark が返る点、または getTheme が待つ仕組みを持っていない点に触れていれば満たす",
      core_reject: [
        "最初の行から dark が返ると読んでいる",
        "2回目も 未取得 のままだと読んでいる",
        "0 ミリ秒なのですぐ実行されて間に合うと読んでいる",
      ],
    },
  },
  ...
  {
    order: 61,
    title: "setTimeout とイベントループ ─ 出力順を並べ替える",
    language: "js",
    difficulty: 5,
    reading_type: "トレース",
    code: `console.log("A");
...
console.log("E");`,
    ...
    model_answer: `出力は A E C D B の順です。
...
そのあと、先に処理されるほうの列に入った C と D が、登録した順で走ります。最後に setTimeout の B が出ます。`,
    prerequisite: `JavaScript は1本の流れで動きます。いま動いている処理が終わるまで、予約されたものは実行されません。

予約には2つの列があります。Promise の then や queueMicrotask が入る列と、setTimeout が入る列です。前者のほうが必ず優先されます。

同じ列の中では、予約した順に実行されます。`,
    keywords: [
      { match: ["setTimeout", "0 を渡", "0 ミリ秒"] },
      { match: ["最後", "いちばん後", "A E C D B"] },
      { match: ["同期", "すべて終わ", "待ち行列"] },
      { match: ["queueMicrotask", "登録した順", "先に"] },
    ],
    rubric_items: {
      core: "setTimeout に 0 を渡しても最後に実行されるという結論を指していれば満たす",
      ground:
        "同期の処理がすべて終わってから予約されたものが処理される点に触れていれば満たす",
      depth:
        "then と queueMicrotask が setTimeout より先に登録した順で走る点、または出力が A E C D B になる点に触れていれば満たす",
      core_reject: [
        "B が C や D より先に出ると読んでいる",
        "C が A の直後に出ると読んでいる",
        "出力が A B C D E の順になると読んでいる",
      ],
    },
  },
  ...
  {
    order: 62,
    title: "Promise ─ pending / fulfilled / rejected",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
console.log("先に");`,
    ...
    model_answer: `最初の行は 3 ではなく Promise { 3 } と出ます。
...
2つ目は function です。3つ目と4つ目の順にも注意が要ります。resolve がその場で呼ばれていても、then に渡した処理は「先に」より後に走るので、先に → あとで 3 の順になります。`,
    prerequisite: `new Promise((resolve, reject) => …) は、いずれ決まる結果を表すものを作ります。resolve(値) で成功、reject(理由) で失敗が決まります。
...
失敗が決まったものを誰も受け止めないと、あとで警告が出ます。.catch() を1つ書いておけば受け止めたことになります。`,
    keywords: [
      { match: ["Promise", "1つ目"] },
      { match: ["3 ではな", "そのもの", "中の値"] },
      { match: ["入れ物", "then などを通", "取り出す"] },
      { match: ["先に", "あとで", "function"] },
    ],
    rubric_items: {
      core: "最初の行が中の値ではなく Promise そのものを出すという結論を指していれば満たす",
      ground:
        "Promise が値そのものではなく いずれ決まる結果を表すものである点に触れていれば満たす",
      depth:
        "then に渡した処理が「先に」より後に走る点、または typeof a.then が function になる点に触れていれば満たす",
      core_reject: [
        "最初の行が 3 を出すと読んでいる",
        "「あとで 3」が「先に」より前に出ると読んでいる",
        "reject した b が catch を書いても落ちると読んでいる",
      ],
    },
  },
  ...
  {
    order: 63,
    title: "then / catch / finally のチェーン",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
  ...
  .finally(() => console.log("finally"));`,
    ...
    model_answer: `catch のあとの then B は走ります。出力は catch 大きすぎます → then B 回復 → finally の3行です。
...
途中で throw されると、そこから先の then は飛ばされて最初の catch へ移ります。ここで then A が飛ばされます。
    ...
    prerequisite: `.then(関数) は前の結果を受け取り、その関数が返した値を次へ渡します。
...
.finally(関数) は成功でも失敗でも最後に実行されます。`,
    keywords: [
      ...
      { match: ["回復", "そのあとも"] },
      { match: ["受け止め", "戻り値", "渡ります"] },
      { match: ["then A", "飛ばさ", "大きすぎます"] },
    ],
    rubric_items: {
      core: "catch のあとの then B も走るという結論を指していれば満たす",
      ground:
        "catch が受け止めた時点で流れが正常に戻りその戻り値が次へ渡る点に触れていれば満たす",
      depth:
        "then A が飛ばされる点、または finally が最後に実行される点に触れていれば満たす",
      core_reject: [
        "then B も飛ばされると読んでいる",
        "then A が実行されると読んでいる",
        "catch のあとは finally しか走らないと読んでいる",
      ],
    },
  },
  ...
  {
    order: 64,
    title: "async / await ─ Promise を上から下に読む",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
console.log("後");`,
    ...
    model_answer: `await で止まるのは loadProfile の中だけで、呼び出し元は待ちません。
...
その後で待っていた結果が決まり「取得 佐藤」が出ます。出力は 前 → 開始 7 → 後 → 取得 佐藤 の順です。`,
    prerequisite: `async を付けた関数の戻り値は、いつでも Promise になります。
...
関数を呼んだ側から見ると、await に差しかかった段階で Promise が返ってきます。そこから先の行は、中の続きが終わるのを待たずに進みます。`,
    keywords: [
      { match: ["await", "loadProfile"] },
      { match: ["呼び出し元", "外側", "待ちません"] },
      { match: ["Promise を返し", "次の行", "手前まで"] },
      { match: ["開始", "取得", "佐藤", "同期"] },
    ],
    rubric_items: {
      core: "await で止まるのは loadProfile の中だけで呼び出し元は待たないという結論を指していれば満たす",
      ground:
        "await に来た時点で Promise を返して呼び出し元へ戻る点に触れていれば満たす",
      depth:
        "出力が 前 → 開始 7 → 後 → 取得 佐藤 になる点、または await の手前までは同期に走る点に触れていれば満たす",
      core_reject: [
        "「取得 佐藤」が「後」より前に出ると読んでいる",
        "「開始 7」が「後」より後に出ると読んでいる",
        "呼ぶと全部終わるまで次の行に進まないと読んでいる",
      ],
    },
  },
  ...
  {
    order: 65,
    title: "await を付け忘れたコードを読む ─ Promise { <pending> } の正体",
    language: "js",
    difficulty: 4,
    reading_type: "ズレ",
...
isPaid(7).then((paid) => console.log(paid));`,
    ...
    model_answer: `await を書いていないので、total には 1200 ではなく Promise がそのまま入ります。
...
buildLabel のほうは「お支払い [object Promise] 円」と出るので、見ればすぐ分かります。ところが isPaid は Promise と 0 を比べることになり、結果は false になるだけです。エラーにもならず数字も出ないので、こちらは気づけません。同じ書き忘れでも、現れ方がまったく違います。`,
    prerequisite: `async を付けた関数の戻り値は、いつでも Promise です。中で return 1200 と書いても、呼んだ側が受け取るのは Promise のほうです。

await を書くと、その Promise の結果が決まるのを待って、中の値を取り出します。書かなければ Promise 自体が変数に入ります。

Promise は数として比べられません。文字に埋め込むと、その旨を表す決まった文字列になります。`,
    // `await` はコードに**書かれていない**（それが問題の主題）ので、
    // スロットに置くと I-817（前提知識にコード外のキーワード）に当たる。
    keywords: [
      { match: ["total", "fetchTotal"] },
      { match: ["1200 ではな", "そのまま入", "Promise のまま"] },
      { match: ["async", "書いていな", "付けていな"] },
      { match: ["object Promise", "false", "isPaid", "気づけ"] },
    ],
    rubric_items: {
      core: "await が無いので total に Promise が入るという結論を指していれば満たす",
      ground:
        "fetchTotal が async なので戻り値がいつでも Promise である点に触れていれば満たす",
      depth:
        "buildLabel は表示で分かるが isPaid は false になるだけで気づけない点に触れていれば満たす",
      core_reject: [
        "total に 1200 が入ると読んでいる",
        "await が無いと実行時エラーになると読んでいる",
        "isPaid が true を返すと読んでいる",
      ],
    },
  },
  ...
  {
    order: 66,
    title: "async 関数の戻り値は必ず Promise になる",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
nested().then((v) => console.log("nested", v));`,
    ...
    model_answer: `1行目は true です。async を付けた関数は、中で何を return しても Promise が返ります。
...
throw した fail は、その場で例外が飛ぶのではなく、失敗が決まった Promise になります。だから catch で受けられます。Promise を return した nested は二重に包まれることはなく、then には中の「入れ子」が渡ります。続く出力は ok 文字列 → fail 失敗 → nested 入れ子 です。`,
    prerequisite: `async を付けた関数の戻り値は、いつでも Promise です。
...
return するものが Promise だった場合は、そのまま入れ子にはならず、中の結果がそのまま外へ渡ります。`,
    keywords: [
      { match: ["async", "ok"] },
      { match: ["Promise", "true"] },
      { match: ["何を return", "中身は then", "呼んだ側"] },
      { match: ["fail", "nested", "二重"] },
    ],
    rubric_items: {
      core: "async を付けた関数は中で何を return しても Promise を返すという結論を指していれば満たす",
      ground: "1行目の ok() instanceof Promise が true になる点に触れていれば満たす",
      depth:
        "throw した fail が失敗の決まった Promise になる点、または Promise を return した nested が二重に包まれない点に触れていれば満たす",
      core_reject: [
        "ok() が文字列そのものを返すと読んでいる",
        "fail() が呼んだ時点で例外を投げると読んでいる",
        "nested() の then に Promise が渡ると読んでいる",
      ],
    },
  },
  ...
  {
    order: 67,
    title: "await × try/catch のエラーハンドリング",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
withoutAwait()
  ...
  .catch((e) => console.log("B catch", e.message));`,
    ...
    model_answer: `withoutAwait の try / catch は、この失敗を受け止められません。
...
withAwait のほうは await があるので、その行で失敗が起きて catch に入り、「A catch で受けた: 通信に失敗」になります。`,
    prerequisite: `await のある行で失敗が決まると、その行で例外が起きます。だから try { … } catch { … } の中に入れておけば受け取れます。
...
async 関数が Promise を return すると、外から見た結果はその Promise の結果になります。`,
    keywords: [
      { match: ["withoutAwait", "await を書", "try"] },
      { match: ["受け止め", "catch には入りません", "捕まえ"] },
      { match: ["抜けたあと", "その場では", "決まるのは後"] },
      { match: ["withAwait", "通信に失敗", "呼び出し側"] },
    ],
    rubric_items: {
      core: "withoutAwait の try / catch が失敗を受け止められないという結論を指していれば満たす",
      ground:
        "await が無いので try を抜けたあとに結果が決まる点に触れていれば満たす",
      depth:
        "呼び出し側の catch が受けることになる点、または withAwait のほうは catch に入る点に触れていれば満たす",
      core_reject: [
        "withoutAwait でも catch で受けたという文字列が返ると読んでいる",
        "withAwait が失敗のまま外へ出ると読んでいる",
        "try の中に書けば await が無くても受け止められると読んでいる",
      ],
    },
  },
  ...
  {
    order: 68,
    title: ".then() の中で throw したエラーはどこへ行くか",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
Promise.resolve("start")
  ...
  .catch((e) => {
    ...
    model_answer: `同じ then の第2引数は、その then の第1引数が投げた例外を受けません。
...
出力は2行ですが、**後半のほうが先に出ます。** 前半は throw と catch のあいだに then が1つ挟まっているぶん、そこを通り抜けるのに1段よけいにかかるためです。`,
    prerequisite: `.then(成功のとき, 失敗のとき) のように、then は関数を2つ受け取れます。2つ目は、その then に入ってくる時点で既に失敗していた場合に呼ばれます。
...
.catch(関数) は .then(undefined, 関数) と同じ意味です。`,
    keywords: [
      { match: ["第2引数", "同じ then"] },
      { match: ["受けません", "呼ばれません", "後ろの catch"] },
      { match: ["あとに続く", "先へ渡", "移ります"] },
      { match: ["次の then", "飛ばさ", "2行"] },
    ],
    rubric_items: {
      core: "同じ then の第2引数はその then の第1引数が投げた例外を受けないという結論を指していれば満たす",
      ground:
        "投げられた例外があとに続く catch へ渡る点に触れていれば満たす",
      depth:
        "前半の連なりで「次の then」が飛ばされる点、または出力が2行だけになる点に触れていれば満たす",
      core_reject: [
        "「同じ then の第2引数」が出力されると読んでいる",
        "「次の then」が実行されると読んでいる",
        "後半の連なりでは何も出力されないと読んでいる",
      ],
    },
  },
  ...
  {
    order: 69,
    title: "catch されない Promise ─ 本番ログに何が出るか",
    language: "js",
    difficulty: 5,
    reading_type: "ズレ",
...
console.log("画面の処理は続きます");`,
    ...
    model_answer: `失敗を誰も受け止めていないので、行き場の無い拒否として扱われます。
...
出力は「画面の処理は続きます」だけです。「送信しました」は出ません。コメントは「画面を止めない」と書いていますが、受け止められなかった拒否は実行環境が記録し、Node.js の既定ではプロセスそのものが終わります。むしろ止める側に働きます。catch を1つ足せば意図どおりになります。`,
    prerequisite: `.then(関数) に渡せるのは、うまくいった場合の処理です。うまくいかなかった場合の処理は .catch(関数) などで別に用意します。
...
.then(...) が返すものも Promise なので、その先で受け取ることができます。`,
    keywords: [
      { match: ["then", "受け止め", "受ける口"] },
      { match: ["行き場", "拒否", "誰も"] },
      { match: ["失敗が決まった", "postToServer"] },
      { match: ["画面の処理は続きます", "送信しました", "catch を足"] },
    ],
    rubric_items: {
      core: "失敗を誰も受け止めていないので行き場の無い拒否になるという結論を指していれば満たす",
      ground:
        "then にうまくいったときの処理しか渡しておらず失敗を受ける口が無い点に触れていれば満たす",
      depth:
        "「送信しました」は出ず「画面の処理は続きます」だけが出る点、または catch を1つ足せば意図どおりになる点に触れていれば満たす",
      core_reject: [
        "失敗が握りつぶされて何も起きないと読んでいる",
        "「送信しました」が出ると読んでいる",
        "sendMetrics を呼んだ行で例外が飛んでそこで止まると読んでいる",
      ],
    },
  },
  ...
  {
    order: 70,
    title: "Promise.all / allSettled / race ─ 1つ失敗したら残りはどうなるか",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
...
Promise.all([task("A", 10, false), task("B", 5, true), task("C", 20, false)])
  ...
  .catch((e) => console.log("all catch", e.message));`,
    question:
      "このコードを実行すると何がどの順で出力されますか。その順になる理由も説明してください。",
    model_answer: `1つ失敗しても、走っている残りの処理は止まりません。
...
出力は 完了 B → all catch B が失敗 → 完了 A → 完了 C の順です。B が5ミリ秒で失敗した時点で all catch に入りますが、10ミリ秒の A と20ミリ秒の C はそのまま最後まで走ります。成功したぶんの結果は受け取れません。`,
    prerequisite: `Promise.all([…]) は、渡したものがすべて成功したときにまとめた結果を返します。1つでも失敗すると、その時点で失敗が決まります。
...
いずれも結果の受け取り方を決めるだけで、渡したものの実行そのものには手を出しません。`,
    keywords: [
      { match: ["Promise.all", "1つ失敗"] },
      { match: ["止まりません", "走り続け", "そのまま最後まで"] },
      { match: ["止める仕組み", "最初の失敗", "結果が決まる"] },
      { match: ["完了 A", "完了 C", "受け取れな"] },
    ],
    rubric_items: {
      core: "1つ失敗しても走っている残りの処理は止まらないという結論を指していれば満たす",
      ground:
        "all が返すものは最初の失敗で決まるだけで動いているものを止める仕組みが無い点に触れていれば満たす",
      depth:
        "出力が 完了 B → all catch → 完了 A → 完了 C の順になる点、または成功したぶんの結果は受け取れない点に触れていれば満たす",
      core_reject: [
        "B が失敗した時点で A と C も止まると読んでいる",
        "「完了 A」が「完了 B」より先に出ると読んでいる",
        "all の then が A と C の結果を受け取ると読んでいる",
      ],
    },
  },
  ...
  {
    order: 71,
    title: "逐次と並列 ─ await をどこに置くかで速度が変わる",
    language: "js",
    difficulty: 4,
    reading_type: "影響",
...
async function parallel() {
  ...
  return [await pa, await pb, await pc];
    ...
    model_answer: `かかる時間が約90ミリ秒から約30ミリ秒に短くなります。戻り値は同じ配列のままです。
...
変わらないものもあります。戻り値の並びは同じですし、3つそれぞれの処理内容も変わりません。ただしこの書き換えができるのは、3つが互いに依存していない場合だけです。前の結果を次に渡している場合は、先に始めること自体ができません。`,
    prerequisite: `await は、その Promise の結果が決まるまでその関数の中の続きを止めます。
...
setTimeout を使った delay(ミリ秒, 値) は、指定した時間が過ぎてからその値で成功が決まります。`,
    keywords: [
      ...
      { match: ["90", "3分の1", "短く"] },
      { match: ["終わってから", "先に始め", "積み上が"] },
      { match: ["同じ配列", "並びは同じ", "依存"] },
    ],
    rubric_items: {
      core: "かかる時間が約90ミリ秒から約30ミリ秒に短くなるという結論を指していれば満たす",
      ground:
        "sequential は前の await が終わってから次を呼び始める点に触れていれば満たす",
      depth:
        "戻り値の配列は変わらない点、または3つが互いに依存していない場合にしか使えない点に触れていれば満たす",
      core_reject: [
        "どちらも同じ時間だけかかると読んでいる",
        "下のほうが遅くなると読んでいる",
        "戻り値の並びが変わると読んでいる",
...
async function loadOrders(userId) {
  try {
    ...
    model_answer: `うまくいかない場合でも同じ形を返すことで、呼ぶ側がいつも同じ扱いをできるようにするためです。
...
res.ok を見ているのは、通信そのものが成功していても中身が失敗でありうるからです。サーバーがエラーを返しても失敗にはならないので、ここを見ないと見落とします。reason があるぶん、握りつぶしにもなっていません。`,
    prerequisite: `fetch(URL) は通信の結果を表す Promise を返します。サーバーが 404 や 500 を返しても失敗にはなりません。届かなかったときだけ失敗になります。
...
ステージ46（テキストとデータの往復）とステージ57（try の範囲）で見たことが、この1つのコードに同時に出てきます。`,
    keywords: [
      { match: ["orders", "reason"] },
      { match: ["同じ形", "同じ扱い", "どの経路でも"] },
      { match: ["res.ok", "空だけ", "分けて"] },
      { match: ["握りつぶ", "区別", "呼ぶ側"] },
    ],
    rubric_items: {
      core: "うまくいかない場合でも同じ形を返して呼ぶ側が同じ扱いをできるようにする意図を指していれば満たす",
      ground:
        "orders がどの経路でも配列である点に触れていれば満たす",
      depth:
        "reason を分けることで空になった理由が残る点、または res.ok を見ないとサーバー側の失敗を見落とす点に触れていれば満たす",
      core_reject: [
        "例外を投げないのは失敗を隠すためだと読んでいる",
        "reason が成功したときにも使われると読んでいる",
        "fetch が失敗すると自動で catch に入るので res.ok は不要だと読んでいる",
      ],
    },
  },
];

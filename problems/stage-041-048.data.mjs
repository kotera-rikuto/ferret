// ステージ 41〜48 の問題データ（第6章 標準機能の引き出しを開ける）。
//
// この章に螺旋の回収は無いが、隣接する繋がりが2本ある。
//   42 ← 6（+ の二つの顔）… 「数字に見えるものが数とは限らない」の続き
//   46 → 72（fetch）      … こちらが回収元になる。72 を作るときはここを読むこと
//
// 投入前に必ず次の2つを回すこと。
//   node problems/preflight.mjs problems/stage-041-048.data.mjs
//   node problems/run-code.mjs  problems/stage-041-048.data.mjs

export const problems = [
  {
    order: 41,
    title: "文字列メソッド ─ 分解・結合・整形・検索",
    language: "js",
    difficulty: 3,
    reading_type: "トレース",
...
console.log(raw.replace(" ", "_"));`,
    ...
    model_answer: `最後の行は、いちばん前の空白1つだけが置き換わって「_在庫, ,急ぎ ,, 在庫 」になります。
...
parseTags のほうは、split でできた空の要素を trim と filter が落とすので [ '在庫', '急ぎ', '在庫' ] の3件になります。同じものをまとめる処理は無いので、在庫 は2つ残ります。`,
    prerequisite: `文字列.split(",") は区切り文字で分けた配列を返します。区切りが連続していると、その間に空の要素ができます。
...
文字列.replace(探すもの, 置き換えるもの) は新しい文字列を返します。探すもの側に文字列を書いたときと正規表現を書いたときで、置き換わる範囲が変わります（正規表現は次のステージ）。`,
    keywords: [
      { match: ["replace", "最後の行"] },
      { match: ["1か所", "1つだけ", "いちばん前"] },
      { match: ["あとの空白", "残ります", "1か所しか"] },
      { match: ["在庫", "3件", "同じもの"] },
    ],
    rubric_items: {
      core: "replace が最初に見つかった1か所だけを置き換えるという結論を指していれば満たす",
      ground:
        "探すもの側に文字列を書いた場合はあとの空白が残る点に触れていれば満たす",
      depth:
        "parseTags の結果が 在庫 / 急ぎ / 在庫 の3件になる点、または同じものをまとめる処理が無いので 在庫 が2つ残る点に触れていれば満たす",
      core_reject: [
        "replace がすべての空白を置き換えると読んでいる",
        "replace が raw そのものを書き換えると読んでいる",
        "parseTags が同じものをまとめて2件になると読んでいる",
      ],
    },
  },
  ...
  {
    order: 42,
    title: "数値の罠 ─ Number / parseInt / NaN と浮動小数点の誤差",
    language: "js",
    difficulty: 4,
    reading_type: "ズレ",
...
console.log(0.1 + 0.2 === 0.3);`,
    ...
    model_answer: `壊れた入力が混じっているのに matched が true になります。
...
2つ目の出力は false です。0.1 + 0.2 はぴったり 0.3 にならないため、=== では確かめられません。`,
    prerequisite: `parseInt(文字列, 10) は文字列を10進の整数として読みます。先頭から読める範囲だけを読み、途中で数と見なせない文字が現れたらそこで打ち切ります。1文字も読めなければ NaN になります。

Number(文字列) は全体が数として読めるかを見るので、途中で打ち切ることはありません。

コンピュータは 0.1 や 0.2 のような値を2進数で近似して持つため、書いたとおりの値にはなりません。`,
    keywords: [
      { match: ["parseInt", "0.5", "80円"] },
      { match: ["true", "一致", "通って"] },
      { match: ["時点で止ま", "潰れ", "そこまでしか"] },
      { match: ["0.1", "false", "ぴったり"] },
    ],
    rubric_items: {
      core: "壊れた入力なのに matched が true になるという結論を指していれば満たす",
      ground:
        "parseInt が途中で止まるため \"0.5\" が 0 になっている点に触れていれば満たす",
      depth:
        "0.1 + 0.2 === 0.3 が false になり小数を === で確かめられない点に触れていれば満たす",
      core_reject: [
        "matched が false になると読んでいる",
        "\"80円\" のところで実行時エラーになると読んでいる",
        "\"0.5\" が 0.5 として足されると読んでいる",
      ],
    },
  },
...
for (const code of codes) {
  console.log(code, patternA.test(code), patternB.test(code));
    ...
    model_answer: `patternA ははじまりと終わりを固定していないので、文字列の一部にその形が見つかれば true になります。
...
出力は3行で、AB-1234 は両方 true、xAB-1234x は patternA だけ true、AB-12345 も patternA だけ true です。最後は末尾に数字が1つ余っているので patternB では false になります。`,
    prerequisite: `/…/ で囲んだものを正規表現といい、文字の並びの形を表します。
...
^ は文字列のはじまり、$ は終わりを表します。これらを書くかどうかで、test が「どこかにあるか」を見るのか「これがすべてか」を見るのかが変わります。`,
    keywords: [
      { match: ["patternA", "はじまりと終わり", "アンカー"] },
      { match: ["一部", "含まれ", "余計な文字"] },
      { match: ["全体", "^ と $", "囲んで"] },
      { match: ["AB-12345", "patternB", "false"] },
    ],
    rubric_items: {
      core: "patternA は文字列の一部に当てはまれば true になるという結論を指していれば満たす",
      ground:
        "patternA には ^ と $ が無いので全体が一致する必要がない点に触れていれば満たす",
      depth:
        "AB-12345 が patternB では false になる点、または3行の判定が並ぶ順に触れていれば満たす",
      core_reject: [
        "patternA が xAB-1234x で false になると読んでいる",
        "patternB が AB-12345 で true になると読んでいる",
        "2つのパターンの結果が3行とも同じになると読んでいる",
      ],
    },
  },
...
console.log(masked);`,
    ...
    model_answer: `g を付けた match では、括弧で取り出した部分は返らず、当てはまった全体だけが並びます。
...
masked のほうは replace に g が付いているので該当箇所を残らず置き換え、order=A-1001 user=U-**** order=A-1002 になります。`,
    prerequisite: `正規表現の後ろに付く g は「当てはまるところを全部たどる」という指定です。
...
文字列.replace(正規表現, 置き換えるもの) は、g があれば該当箇所を残らず置き換えます。`,
    keywords: [
      { match: ["ids", "g を付け", "グローバル"] },
      { match: ["全体", "括弧", "返らず"] },
      { match: ["order=", "含んだ", "そのまま並"] },
      { match: ["first", "A-1001", "masked"] },
    ],
    rubric_items: {
      core: "g を付けた match では括弧で取り出した部分が返らないという結論を指していれば満たす",
      ground:
        "ids に order= を含んだ文字列が入っている点に触れていれば満たす",
      depth:
        "g の無いほうでは first[1] から A-1001 が取れる点、または masked が該当箇所を残らず置き換える点に触れていれば満たす",
      core_reject: [
        "ids に A-1001 と A-1002 だけが入ると読んでいる",
        "first[1] が order=A-1001 になると読んでいる",
        "masked の置き換えが最初の1か所だけになると読んでいる",
      ],
    },
  },
  ...
  return {
    ...
    model_answer: `数え方に必要な決まりを、自分で書かずに入れ物の側に任せるためです。
...
countByPage のほうを選んでいるのは、名前を入れた順に保ち、名前が文字へ変えられないためです。ふつうのオブジェクトだと、数字らしい名前が付いたページの並び順が入れた順と変わってしまいます。`,
    prerequisite: `new Set() は、同じ値を1つしか持たない入れ物です。add(値) で足し、size で個数が分かります。
...
[...entries()] は、[名前, 値] の組を並べた配列にします。`,
    keywords: [
      { match: ["uniqueUsers", "countByPage"] },
      { match: ["任せ", "自分で書かず", "決まり"] },
      { match: ["size", "add", "人数"] },
      { match: ["並び順", "文字へ変え", "includes"] },
    ],
    rubric_items: {
      core: "数え方に必要な決まりを自分で書かず入れ物の側に任せるという意図を指していれば満たす",
      ground:
        "add を呼ぶだけで同じ値が重ならず size がそのまま人数になる点に触れていれば満たす",
      depth:
        "ふつうのオブジェクトだと名前が文字へ変えられて並び順が変わる点、または配列なら includes で確かめる処理が要る点に触れていれば満たす",
      core_reject: [
        "処理を速くするために選んでいると読んでいる",
        "2つの入れ物は同じもので使い分けに意味は無いと読んでいる",
        "uniqueUsers が訪問回数を数えていると読んでいる",
      ],
    },
  },
  ...
  {
    order: 46,
    title: "JSON.stringify / JSON.parse ─ 文字列とデータの往復",
    language: "js",
    difficulty: 4,
    reading_type: "トレース",
    code: `const draft = {
...
console.log("reviewer" in restored);`,
    ...
    model_answer: `2つ目は string になります。
...
1つ目のテキストには title と savedAt と tags しか入りません。tags は空のオブジェクト {} になり、reviewer と render は項目ごと落ちます。そのため3つ目は {}、4つ目は false になります。`,
    prerequisite: `JSON.stringify(値) はデータを1本のテキストにします。JSON.parse(テキスト) はテキストをデータに戻します。
...
戻す側は、もとが何だったかを知りません。テキストに書かれている形のまま組み立てます。`,
    keywords: [
      { match: ["savedAt", "Date"] },
      { match: ["string", "文字列"] },
      { match: ["Date に戻", "作り直す", "書き出す時点"] },
      { match: ["tags", "空のオブジェクト", "reviewer", "落ち"] },
    ],
    rubric_items: {
      core: "往復すると savedAt が文字列になるという結論を指していれば満たす",
      ground:
        "書き出す時点で Date が文字列へ変えられ JSON.parse が戻さない点に触れていれば満たす",
      depth:
        "tags が空のオブジェクトになる点、または reviewer と render が項目ごと落ちる点に触れていれば満たす",
      core_reject: [
        "restored.savedAt が Date のままだと読んでいる",
        "tags が配列として戻ると読んでいる",
        "reviewer が undefined として残ると読んでいる",
      ],
    },
  },
...
console.log(base.getMonth() + 1, base.getDate());`,
    question:
      "3つの console.log でそれぞれ何が出力されますか。1つ目がその結果になる理由も説明してください。",
    model_answer: `日にあたる引数に 0 を渡すと、ひとつ前の月の最終日になります。
...
3つ目は setMonth(1) で1月31日を2月にしていますが、2月31日という日は無いので3月3日へ送られます。出力は 3 3 です。`,
    prerequisite: `new Date(年, 月, 日) で日付を作れます。月の指定だけは 0 が1月、11 が12月です（getMonth() も同じ数え方）。
...
setMonth(月) は月だけを変えます。日はそのまま残ります。`,
    keywords: [
      { match: ["0 を渡", "月は0"] },
      { match: ["最終日", "ひとつ前の月", "2月28"] },
      { match: ["0始まり", "0 から数え", "1つずれ"] },
      { match: ["setMonth", "3月3日", "2月31日"] },
    ],
    rubric_items: {
      core: "日にあたる引数に 0 を渡すとひとつ前の月の最終日になるという結論を指していれば満たす",
      ground: "月の指定が0始まりで 2 が3月を指す点に触れていれば満たす",
      depth:
        "setMonth(1) で1月31日が3月3日へ送られる点、または1つ目が 2026-2-28 になる点に触れていれば満たす",
      core_reject: [
        "1つ目が 2026-2-1 になると読んでいる",
        "1つ目が 2026-3-1 になると読んでいる",
        "setMonth(1) の結果が2月31日のまま残ると読んでいる",
      ],
    },
  },
  ...
  {
    order: 48,
    title: "タイムゾーン ─ JST と UTC がズレるコードを読む",
    language: "js",
    difficulty: 5,
    reading_type: "ズレ",
...
for (const order of orders) {
  console.log(order.id, toJstKey(order.placedAt));
    ...
    question:
      "コメントに書かれた意図と、実際の動きが食い違っています。2つの出力を示したうえで、どこがどう食い違うかを説明してください。",
    model_answer: `A-2 は日本時間では19日の0時30分ですが、キーは 2026-08-18 になります。
...
A-1 のほうはどちらで見ても18日なので、ずれていません。毎日9時間ぶんの注文が前の日に寄せられる、という形の食い違いです。`,
    prerequisite: `new Date("…Z") の末尾の Z は、その時刻が世界標準時であることを表します。

toISOString() が返す表記は、どの環境で動かしても世界標準時のものです。実行している場所の時間帯には合わせません。

日本の時間帯は世界標準時より進んでいるので、同じ瞬間を指していても日付が変わることがあります。`,
    keywords: [
      { match: ["A-2", "2件目", "15時30分"] },
      { match: ["19日", "翌日", "前日"] },
      { match: ["toISOString", "9時間", "考慮され"] },
      { match: ["A-1", "15時以降", "ずれていない"] },
    ],
    rubric_items: {
      core: "日本時間では19日の注文が18日として数えられるという結論を指していれば満たす",
      ground:
        "toISOString が常に世界標準時の日付を返し9時間ぶんの差が入っていない点に触れていれば満たす",
      depth:
        "A-1 のほうはどちらで見ても18日なのでずれていない点、または毎日9時間ぶんの注文だけが前の日に寄せられる点に触れていれば満たす",
      core_reject: [
        "2件とも日本時間でも18日なので問題ないと読んでいる",
        "A-1 のほうがずれていると読んでいる",
        "toISOString が実行環境の時間帯に合わせて変わると読んでいる",
      ],
    },
  },
];

...
handle({ kind: "keydown", code: "Enter" });',
  'src/handle.ts:17:8 - error TS2345: Argument of type ''{ kind: "keydown"; code: string; }'' is not assignable to parameter of type ''Event''.
...
17 handle({ kind: "keydown", code: "Enter" });
  ...
  '本当の食い違いは kind ではなく、key を渡していないことです。
...
長い候補の並びでは、出力に出ている候補が原因とは限りません。**まず渡した値と各候補を自分で突き合わせ、どれに近いかを決めてから読む**必要があります。ここでは kind が keydown なので、比べるべきは2番目の候補です。',
  '[{"match":["code","渡していな","key を渡して"]},{"match":["どれにも当てはまら","3つの候補","候補のどれ"]},{"match":["いちばん最初","click との比較","先頭"]},{"match":["2番目","自分で突き合わせ","原因とは限らな"]}]'::jsonb,
  '{"core":"本当の食い違いが kind ではなく key を渡していないことだという結論を指していれば満たす","depth":"出力に出ている候補が原因とは限らない点、または比べるべきが2番目の候補である点に触れていれば満たす","ground":"渡した値が3つの候補のどれにも当てはまっていない点に触れていれば満たす","core_reject":["kind の値そのものが誤っていると読んでいる","Event に click しか含まれていないと読んでいる","出力に出ている候補が必ず原因だと読んでいる"]}'::jsonb,
  '| でつないだ型は「そのどれか」を表します。渡した値がどれか1つに合えば通ります。
...
interface Repository<T> {
  ...
  'Timestamps を切り出したのは、同じ2項目を2か所に書かずに済ませるためです。
...
Repository を2回書いているのは別の理由です。**同じ名前の interface は自動的に1つにまとまります。** これは type には無い性質で、あとから項目を足せることを意味します。自分のコードで分ける意味は薄いですが、**外部のライブラリの型に自分の分を足したいとき**にこの性質が要ります。',
  '[{"match":["Timestamps","2か所に書かず","共通"]},{"match":["Article","Comment","両方の項目"]},{"match":["1つにまとま","自動的に","interface は"]},{"match":["外部のライブラリ","あとから","type には無い"]}]'::jsonb,
  '{"core":"Timestamps を切り出したのが同じ項目を2か所に書かずに済ませるためだという意図を指していれば満たす","depth":"同じ名前の interface が1つにまとまる点、または外部のライブラリの型に足したいときにその性質が要る点に触れていれば満たす","ground":"& でつなぐと両方の項目をあわせ持つ形になる点に触れていれば満たす","core_reject":["Timestamps が速さのために切り出されていると読んでいる","Repository を2回書いているのは書き間違いだと読んでいる","type でも同じ名前で複数回書けると読んでいる"]}'::jsonb,
  'type 名前 = … は型に名前を付けます。A & B は「A と B の項目をあわせ持つ」という意味になります。
...
values (
  ...
  4,
  'トレース',
  'function firstOr<T>(list: T[], fallback: T): T {
...
console.log(a, b, c);',
  ...
  'a では T が number、b では string、c でも string になります。
...
出力は 1 x 空でした です。a は配列に中身があるので先頭の 1、b も先頭の x、c は空なので fallback がそのまま返ります。',
  '[{"match":["number","string"]},{"match":["渡した引数","a では","b では"]},{"match":["空でした","fallback","空で"]},{"match":["1 x 空でした","先頭","2か所"]}]'::jsonb,
  '{"core":"T が渡した引数から number や string に決まるという結論を指していれば満たす","depth":"c は配列が空なのでもう1つの引数から決まる点、または出力が 1 x 空でした になる点に触れていれば満たす","ground":"a は配列から b と c は引数の内容から決まる点に触れていれば満たす","core_reject":["T がいつも同じ型に決まると読んでいる","c で T が決められず実行時エラーになると読んでいる","c の出力が空文字になると読んでいる"]}'::jsonb,
  '<T> は「使うときに決まる型」を表す入れ物です。関数の中では、それが何であるかを決めつけずに書けます。
...
Partial は全部を省略可能にしたもの、Readonly は全部を書き換え不可にしたもの、Pick は名前を挙げた2つだけを残したもの、Omit は挙げた1つを取り除いたものです。つまり OnlyView と NoDebug は結果として同じ形になります。**もとの defaults を直せば、5つとも自動的に付いてきます。**',
  '[{"match":["typeof defaults","値から型","取り出し"]},{"match":["SettingKey","集める","perPage"]},{"match":["OnlyView","NoDebug","同じ形"]},{"match":["defaults を直せば","自動的に付いて","ひとつから"]}]'::jsonb,
  '{"core":"5つの型がすべて defaults ひとつから組み立てられているという結論を指していれば満たす","depth":"OnlyView と NoDebug が結果として同じ形になる点、または defaults を直せば5つとも付いてくる点に触れていれば満たす","ground":"typeof が値から型を取り出し keyof が項目名を集めている点に触れていれば満たす","core_reject":["5つの型がそれぞれ独立に書かれていると読んでいる","keyof が項目の値を集めると読んでいる","Omit が挙げた名前だけを残すと読んでいる"]}'::jsonb,
  'typeof 値 を型の位置に書くと、その値の形を型として取り出せます。値と型を二重に書かずに済みます。
...
そのおかげで if の中では r.data が読めます。ApiErr には data がないので、絞り込みが効いていなければ書けない行です。else 側には ApiErr だけが残るので r.message が読めます。',
  '[{"match":["r is ApiOk","戻り値に書","isOk"]},{"match":["伝える","呼び出し側","ただの boolean"]},{"match":["r.data","絞ら","読める"]},{"match":["a,b","権限がありません","r.message"]}]'::jsonb,
  '{"core":"r is ApiOk が true のとき値が ApiOk だと呼び出し側に伝える書き方だという結論を指していれば満たす","depth":"if の中で r.data が読める点、または出力が a,b と 失敗: 権限がありません になる点に触れていれば満たす","ground":"ただの boolean にすると if の中で絞られない点に触れていれば満たす","core_reject":["r is ApiOk が実行時に型を変えていると読んでいる","ただの boolean でも同じように絞られると読んでいる","2つ目の出力が a,b になると読んでいる"]}'::jsonb,
  '| でつないだ型は「そのどれか」を表します。どちらであるかを確かめるまで、片方にしかない項目は読めません。
...
ステージ11 の switch は「どの枝を通るか」を追うものでしたが、ここでは**枝を通ること自体が型を絞る**という働きが加わります。候補を1つ足して case を書き忘れると、戻り値に undefined が混ざって指摘されます。',
  '[{"match":["status","重なりま","4つの候補"]},{"match":["その1つ","1つに","case を通る"]},{"match":["rows","message","その中でだけ"]},{"match":["待機中","3件","通信断","書き忘れ"]}]'::jsonb,
  '{"core":"case を通ると state が1つの候補に絞られるという結論を指していれば満たす","depth":"done の中でだけ rows が読める点、または出力が 待機中 / 3件 / 失敗: 通信断 になる点に触れていれば満たす","ground":"status を4つの候補すべてが持っていて値が重ならない点に触れていれば満たす","core_reject":["どの case の中でも4つの候補すべてとして扱われると読んでいる","2つ目の出力が rows の中身そのものになると読んでいる","status を見ても候補は絞られないと読んでいる"]}'::jsonb,
  'すべての候補が同じ名前の項目を持ち、その値が重ならないとき、その項目を見るだけでどの候補かを決められます。
...
good().then((v) => console.log(v));',
  ...
  'bad の n は number ではなく Promise<number> になります。
...
good では await が包みを開けるので、n は number になり n * 2 が通ります。出力は 84 です。**65 では同じ書き忘れが動かすまで気づけませんでしたが、型を書いておくと書いた時点で止まります。** これが型注釈の効き方のひとつです。',
  '[{"match":["Promise<number>","包みごと","await を付けないと"]},{"match":["n * 2","指摘","掛けられな"]},{"match":["good","84","開ける"]},{"match":["書いた時点","65 では","止まり"]}]'::jsonb,
  '{"core":"bad の n が number ではなく Promise<number> になるという結論を指していれば満たす","depth":"good の出力が 84 になる点、またはステージ65 と違って書いた時点で止まる点に触れていれば満たす","ground":"async の戻り値が Promise に包まれ await を付けないと包みごと入る点に触れていれば満たす","core_reject":["bad の n にも 42 が入ると読んでいる","await が無くても掛け算はできると読んでいる","good の出力が 42 になると読んでいる"]}'::jsonb,
  'async を付けた関数の戻り値は、いつでも Promise です。Promise<T> は「いずれ T になるもの」を表します。
...
values (
  ...
  5,
  '意図',
  'type Config = { apiBase: string; retries: number };
...
export function parseAny(raw: string): Config {
  ...
  'どちらも「型検査では確かめようがないことを、書いた側が引き受ける」という宣言です。
...
loaded! は、値が null でないと書き手が知っているためです。読み込む前に使われないという前提はコメントにありますが、型検査はそれを読めません。**どちらも前提が崩れた瞬間に実行時の失敗になります。** parseAny のように as も ! も書かなければ、その場で指摘されます。',
  '[{"match":["as Config","JSON.parse","型からは分からな"]},{"match":["引き受け","責任","書いた側"]},{"match":["loaded!","null でない","コメント"]},{"match":["静かに通","前提が崩れ","parseAny"]}]'::jsonb,
  '{"core":"型検査では確かめようがないことを書いた側が引き受ける宣言だという意図を指していれば満たす","depth":"確かめているわけではないので前提が崩れると実行時の失敗になる点に触れていれば満たす","ground":"JSON.parse が返すものが型からは分からない点、または loaded が null でないと書き手だけが知っている点に触れていれば満たす","core_reject":["as が実行時に値を変換していると読んでいる","! が null のときに既定値を入れていると読んでいる","どちらも書けば安全になると読んでいる"]}'::jsonb,
  '値 as 型 は「この値をこの型として扱う」と書き手が言い切る書き方です。実行時に何かを確かめるわけではありません。
...
values (
  ...
  5,
  '仕様',
  '// node_modules/@example/queue/index.d.ts
...
onFailed は失敗したときに呼ばれる関数を受け取り、**戻り値がまた関数**です。この形は「登録を解除するための関数を返す」約束によく使われます。JobOptions はどちらの項目も省略可能で、コメントに既定値まで書かれています。close も Promise を返すので、終わるのを待てます。',
  '[{"match":["new Queue","add","積み"]},{"match":["readonly","想定されて","id と payload"]},{"match":["cancel","取り消せたか","boolean"]},{"match":["onFailed","また関数","解除"]}]'::jsonb,
  '{"core":"型定義だけからこのライブラリの使い方が読み取れるという結論を指していれば満たす","depth":"cancel の戻り値から取り消せたかどうかが返ると読める点、または onFailed が解除用の関数を返すと読める点に触れていれば満たす","ground":"new Queue で作り add で仕事を積むという流れに触れていれば満たす","core_reject":["実装を読まないと使い方は分からないと読んでいる","readonly の項目も書き換えてよいと読んでいる","onFailed が失敗の件数を返すと読んでいる"]}'::jsonb,
  '.d.ts は型だけを書いたファイルです。実装は別にあり、使う側はこの宣言だけを見て書きます。
...
export function tryParseOrder(raw: unknown) {
  ...
  'OrderSchema ひとつで、実行時の検査と型の両方を決めています。
...
parse は合わなければ例外を投げます。safeParse は投げずに、成否と中身を持ったものを返します。**呼ぶ側で分岐したいなら safeParse、そこで止めてよいなら parse** という使い分けになります。なお status には既定値があるので、渡されていなくても結果には入ります。',
  '[{"match":["OrderSchema","同時に","両方"]},{"match":["z.infer","手で書いて","二重に"]},{"match":["parse","safeParse","例外"]},{"match":["uuid","正の整数","既定値","status"]}]'::jsonb,
  '{"core":"1つの定義が実行時の検査と型の両方を決めているという結論を指していれば満たす","depth":"parse が例外を投げ safeParse が成否を返す使い分けの点、または status の既定値が結果に入る点に触れていれば満たす","ground":"型を z.infer で取り出していて手で書いていない点に触れていれば満たす","core_reject":["型と検査を別々に書いていると読んでいる","parse と safeParse の違いが無いと読んでいる","この定義が実行時には何もしないと読んでいる"]}'::jsonb,
  'zod は、値の形を1か所に書いておき、実行時にその形かどうかを確かめるためのライブラリです。

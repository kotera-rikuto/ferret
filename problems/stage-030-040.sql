-- ステージ30〜40 投入
-- 出典: problems/stage-030-040.data.mjs / 設計: problems/stage-030-040.md
...
console.log(waiting);',
  ...
  'waiting からは3件が消えて、[ ''u4'', ''u5'' ] の2件だけになります。
...
batch の中身自体はコメントどおりで [ ''u1'', ''u2'', ''u3'' ] になります。食い違うのは、もとの待ち行列が残らない点だけです。',
  '[{"match":["splice","waiting"]},{"match":["2件","u4","減っ"]},{"match":["同時に","取り出すと","もとの配列から"]},{"match":["batch","u1","コメントどおり"]}]'::jsonb,
  '{"core":"waiting から3件が消えて2件になるという結論を指していれば満たす","depth":"batch の中身自体はコメントどおり u1 から u3 になっており食い違うのがもとの配列だけである点に触れていれば満たす","ground":"splice が取り出すと同時にもとの配列から要素を抜く点に触れていれば満たす","core_reject":["waiting が5件のまま残ると読んでいる","batch が空の配列になると読んでいる","splice ももとの配列を変えないと読んでいる"]}'::jsonb,
  '配列のメソッドには、もとの配列に手を入れるものと、手を入れずに新しい配列を返すものの2種類があります。
...
values (
  ...
  '1件ずつ送るという行為そのものが目的で、変換した結果を集めた配列を作りたいわけではないからです。
...
try を各件の中に置いてあるのも同じ理由です。1件が失敗しても残りの送信は続き、最後に失敗したものだけをまとめて返す形になっています。',
  '[{"match":["送信","送る","send"]},{"match":["行為そのもの","副作用","作りたいわけではな"]},{"match":["failed","返しておらず","外側"]},{"match":["失敗したものだけ","try","残りの送信"]}]'::jsonb,
  '{"core":"送るという行為そのものが目的で結果の配列を作るためではないという意図を指していれば満たす","depth":"集めたいのが失敗したものだけである点、または try を各件に置くことで1件の失敗で全体が止まらない点に触れていれば満たす","ground":"コールバックが値を返しておらず必要なものを外側の failed に貯めている点に触れていれば満たす","core_reject":["送信結果を集めた配列を作るために回していると読んでいる","失敗した時点で処理を打ち切るために回していると読んでいる","そのほうが速いのでこう書いたと読んでいる"]}'::jsonb,
  '配列.forEach(関数) は、要素を1つずつ関数に渡して実行します。関数が返した値は使われず、forEach 自体も何も返しません。
...
console.log(lines[1]);',
  ...
  '後半は「2. 発注」のままで、あとから entries[1].name を書き換えても反映されません。
...
lines の長さはもとと同じ 3 なので、前半の出力は 3 3 になります。',
  '[{"match":["lines","map"]},{"match":["反映され","書き換えても","そのまま","変わらな"]},{"match":["文字列","その時点","作り終え"]},{"match":["同じ長さ","3 3","i + 1"]}]'::jsonb,
  '{"core":"lines の2番目が書き換え前のままで反映されないという結論を指していれば満たす","depth":"lines の長さがもとと同じ 3 になる点、または前半の出力が 3 3 になる点に触れていれば満たす","ground":"map がその時点の値から文字列を作り終えており lines に文字列そのものが入っている点に触れていれば満たす","core_reject":["lines の2番目が「2. 発注（修正）」になると読んでいる","lines の長さがもとの配列と変わると読んでいる","map がもとの配列を書き換えると読んでいる"]}'::jsonb,
  '配列.map(関数) は、要素を1つずつ関数に渡し、その戻り値を集めた新しい配列を返します。もとの配列には手を入れません。
...
console.log(labels);',
  ...
  'qty が 0 ではない1件目は undefined になり、labels は [ undefined, ''B-200 品切れ'' ] になります。
...
品切れの行のほうは return が書かれているので、意図どおり ''B-200 品切れ'' に整形されます。',
  '[{"match":["qty","1件目","A-100"]},{"match":["undefined","値が入らない"]},{"match":["return","捨て","書かれていない"]},{"match":["品切れ","B-200","意図どおり"]}]'::jsonb,
  '{"core":"qty が 0 でない行が undefined になるという結論を指していれば満たす","depth":"品切れの行のほうは return があるので意図どおり整形される点に触れていれば満たす","ground":"if に入らなかったときの式に return が無く値が捨てられている点に触れていれば満たす","core_reject":["labels の1件目が ''A-100 × 2'' になると読んでいる","品切れの行も undefined になると読んでいる","map が実行時エラーになると読んでいる"]}'::jsonb,
  '配列.map(関数) は、要素ごとに関数を呼び、その戻り値を集めた新しい配列を返します。関数が値を返さなかった要素の位置には、値が無いことを表すものが入ります。
...
console.log(checkStock([]));',
  ...
  '後半では空の配列を渡しているのに allReady が true になります。
...
同じ空の配列でも some のほうは false になります。find は見つからないと undefined を返すので、first は ?? によって「なし」になります。前半は count が 1、first が B-2、allReady が false です。',
  '[{"match":["空の配列","空配列"]},{"match":["allReady","true"]},{"match":["every","1つも","反例"]},{"match":["なし","B-2","undefined","count"]}]'::jsonb,
  '{"core":"空の配列に対して allReady が true になるという結論を指していれば満たす","depth":"前半が count 1 で first が B-2 になる点、または空のとき find が値を返さないので first が「なし」になる点に触れていれば満たす","ground":"every は条件に合わない要素が1つも無ければ true を返す点に触れていれば満たす","core_reject":["空の配列のとき allReady が false になると読んでいる","空の配列のとき first のところで実行時エラーになると読んでいる","空の配列のとき anyShort も true になると読んでいる"]}'::jsonb,
  'filter は条件に合う要素だけを集めた配列、find は条件に合う最初の要素を返します。find で見つからなかったときの結果は、値が無いことを表すものになります。
...
console.log(wrong);',
  '2つの console.log でそれぞれ何が出力されますか。後半がその結果になる理由も説明してください。',
  '後半は wrong のほうで、配列の最初の要素そのもの（オブジェクト）が sum の出発点になります。
...
前半の total は 0 を渡しているので素直に足し合わされて 2500 になります。',
  '[{"match":["wrong","省いた","第2引数を書かない"]},{"match":["最初の要素","先頭の要素","オブジェクトが出発点"]},{"match":["2番目から","1回目が飛","そこから始ま"]},{"match":["2500","object Object"]}]'::jsonb,
  '{"core":"後半では配列の最初の要素そのものが出発点になるという結論を指していれば満たす","depth":"後半の出力が [object Object]800500 になる点、または前半が 2500 になる点に触れていれば満たす","ground":"第2引数を書かないと配列の先頭が出発点として使われる点に触れていれば満たす","core_reject":["後半も 2500 になると読んでいる","後半が 1300 になると読んでいる","第2引数が無いと実行時エラーになると読んでいる"]}'::jsonb,
  '配列.reduce(関数, 初期値) は、前回までの結果と今の要素を関数に渡し、返った値を次に持ち越します。最後に残った値が全体の結果です。
...
console.log(initial.open);',
  'コメントに書かれた意図と、実際の動きが食い違っています。2つの出力を示したうえで、どこがどう食い違うかを説明してください。',
  'initial.open にも [ ''T-1'', ''T-3'' ] が入ってしまいます。
...
集計そのものは正しく行われていて、closed 側には T-2 が入ります。ステージ4で見た「指し先だけが渡る」形が、ここでも効いています。',
  '[{"match":["initial","acc"]},{"match":["T-1","T-3","書き換わ"]},{"match":["push","そのまま","複製されず"]},{"match":["closed","T-2","集計そのもの"]}]'::jsonb,
  '{"core":"initial の中身も書き換わってしまうという結論を指していれば満たす","depth":"closed 側には T-2 が入っており集計そのものは正しく行われている点に触れていれば満たす","ground":"第2引数に渡した initial が複製されずそのまま acc として使われる点に触れていれば満たす","core_reject":["initial.open が空の配列のまま残ると読んでいる","grouped と initial が別のオブジェクトになると読んでいる","reduce が第2引数を複製してから使うと読んでいる"]}'::jsonb,
  '配列.reduce(関数, 最初の値) は、前回までの結果と今の要素を関数に渡し、返った値を次に持ち越します。渡した最初の値は複製されません。
...
console.log(scores === sorted);',
  ...
  '比較関数を渡していないので、要素は数の大小ではなく文字として比べられます。その結果 10, 100, 25, 9 の順に並びます。
...
出力は [ 10, 100, 25, 9, 1 ] と true になります。',
  '[{"match":["比較関数","引数を渡していない","何も渡さない"]},{"match":["文字として","文字列として","10, 100, 25, 9"]},{"match":["既定","決まっている並べ方"]},{"match":["true","同じ配列","元の配列","push"]}]'::jsonb,
  '{"core":"比較関数が無いので数の大小ではなく文字として並ぶという結論を指していれば満たす","depth":"sort がもとの配列そのものを返すので scores === sorted が true になる点、または push した 1 が scores にも現れる点に触れていれば満たす","ground":"sort が既定で要素を文字として比べる点に触れていれば満たす","core_reject":["9, 10, 25, 100 の順に並ぶと読んでいる","scores がもとの順のまま残ると読んでいる","scores === sorted が false になると読んでいる"]}'::jsonb,
  '配列.sort() は要素を並べ替えます。引数に関数を渡すと、2つを比べてどちらを前に置くかを自分で決められます。渡さなかったときの並べ方は決まっており、数として比べるとは限りません。
...
console.log(ordered.map((t) => t.title));',
  ...
  'priority が同じときだけ dueDays で比べます。
...
出力は [ ''請求書送付'', ''月次報告'', ''在庫補充'', ''棚卸し'' ] です。priority が 2 の2件は dueDays の小さい 在庫補充 が先に来ます。priority も dueDays も同じ2件は、もとの並び順のまま残ります。',
  '[{"match":["priority","dueDays"]},{"match":["同じとき","同点","0 のとき"]},{"match":["||","右側","先に計算"]},{"match":["在庫補充","請求書送付","もとの並び"]}]'::jsonb,
  '{"core":"priority が同じときだけ dueDays で比べるという結論を指していれば満たす","depth":"出力が 請求書送付 / 月次報告 / 在庫補充 / 棚卸し になる点、または両方同じ2件がもとの並び順のまま残る点に触れていれば満たす","ground":"a.priority - b.priority が 0 のときだけ || の右側が使われる点に触れていれば満たす","core_reject":["dueDays だけで並ぶと読んでいる","priority と dueDays を足した値で並ぶと読んでいる","もとの tasks も並べ替わると読んでいる"]}'::jsonb,
  'sort に渡す関数は2つの要素を受け取り、負の数なら前者を前に、正の数なら後者を前に、0 なら順序を変えないという意味の数を返します。
...
console.log(total);',
  ...
  'A-3 は amount が 500 で coupon が 600 なので、map の結果が -100 という負の数になります。
...
filter の時点で canceled の A-2 が外れて2件になり、map で 1000 と -100 になります。reduce で足し合わせるので、出力は 900 です。',
  '[{"match":["A-3","coupon","600"]},{"match":["-100","マイナス","負の数"]},{"match":["map","下限","そのまま返"]},{"match":["900","2件","A-2","filter"]}]'::jsonb,
  '{"core":"クーポンが金額を超えた A-3 が負の数になるという結論を指していれば満たす","depth":"filter で A-2 が外れて2件になる点、または合計が 900 になる点に触れていれば満たす","ground":"map が amount - coupon をそのまま返しており下限を設けていない点に触れていれば満たす","core_reject":["A-3 が 0 として扱われると読んでいる","合計が 1000 になると読んでいる","canceled の A-2 も合計に入ると読んでいる"]}'::jsonb,
  'メソッドをつなげて書くと、前の結果に対して次のメソッドを呼びます。filter で件数が減り、map で1件ごとの形が変わり、reduce で1つの値にまとまります。
...
const allLines = response.orders.flatMap((order) =>
  ...
  '注文1件ごとに明細の配列が返るのを、1本の配列にならすための書き方です。
...
各明細に orderId を足しているのは、平らにしたあとでもどの注文のものか分かるようにするためです。lines が空の A-2 は何も足さないので、結果には残りません。',
  '[{"match":["flatMap","注文1件ごと"]},{"match":["1本","平ら","横並び"]},{"match":["lines","配列の配列","内側"]},{"match":["orderId","A-2","何も足さない"]}]'::jsonb,
  '{"core":"注文ごとに返る配列を1本の配列にならすためという意図を指していれば満たす","depth":"orderId を足すことで平らにしたあとも元の注文が分かるようにしている点、または lines が空の A-2 は結果に残らない点に触れていれば満たす","ground":"内側の map が注文1件につき明細の配列を返している点に触れていれば満たす","core_reject":["明細を注文ごとにまとめ直すために使っていると読んでいる","処理を速くするために使っていると読んでいる","flatMap が入れ子をすべての深さまで開くと読んでいる"]}'::jsonb,
  '配列.flat() は、要素として入っている配列を1段だけ開いて、外側の配列に並べ直します。

...
for (const kind of ["class", "string"]) {
  ...
  '2周目は undefined / undefined / false になります。
...
e instanceof Error も false です。1周目のほうは ValidationError / quantity が不正です / true になります。',
  '[{"match":["文字列","壊れています","2回目"]},{"match":["undefined","取り出せな","入っていな"]},{"match":["throw","Error 以外","そのまま受け取"]},{"match":["instanceof","false","ValidationError"]}]'::jsonb,
  '{"core":"文字列を投げた回は e に name も message も入っていないという結論を指していれば満たす","depth":"e instanceof Error が false になる点、または1周目が ValidationError / quantity が不正です / true になる点に触れていれば満たす","ground":"throw には Error 以外の値も書けて catch がそれをそのまま受け取る点に触れていれば満たす","core_reject":["2周目の message に「壊れています」が入ると読んでいる","文字列を投げると catch されずに落ちると読んでいる","1周目の name が Error になると読んでいる"]}'::jsonb,
  'throw は例外を発生させ、その場で処理を中断します。catch (e) の e には、投げられた値がそのまま入ります。
...
出力の順も同じ理由で決まります。loadRecord(1) は try → finally → 見つかりました、loadRecord(-1) は catch → finally → 見つかりません の順に出ます。戻り値が決まっていても、それが呼び出し元へ戻る前に finally が走るためです。',
  '[{"match":["finally","loadRecordBroken"]},{"match":["捨て","上書き","置き換わ"]},{"match":["return","決めたあと","戻る前"]},{"match":["出力の順","見つかりません","先に走"]}]'::jsonb,
  '{"core":"finally に return を書くと try の return が捨てられるという結論を指していれば満たす","depth":"出力が try → finally → 戻り値 の順になる点、または loadRecord(-1) で catch → finally の順に出る点に触れていれば満たす","ground":"finally が try や catch の return より後に走る点に触れていれば満たす","core_reject":["最後の1行が「見つかりました」になると読んでいる","return のあとなので finally は実行されないと読んでいる","loadRecord(-1) では finally が走らないと読んでいる"]}'::jsonb,
  'try { … } catch (e) { … } finally { … } の finally は、例外が起きても起きなくても最後に走ります。
...
console.log(loadUserPrefs("{壊れた", applyTheme));',
  ...
  '1つ目は設定が壊れていないのに、既定値の { theme: ''light'', perPage: 20 } が返ります。
...
2つ目も同じ戻り値になるので、どちらが起きたのかを呼び出し元から区別できません。catch は受け取った値を一度も見ていないので、記録も残りません。',
  '[{"match":["applyTheme","1つ目","try の範囲"]},{"match":["既定値","light","20"]},{"match":["catch","まとめて","広すぎ","囲まれ"]},{"match":["区別","見分け","記録"]}]'::jsonb,
  '{"core":"1つ目は設定が壊れていないのに既定値が返るという結論を指していれば満たす","depth":"2つ目と戻り値が同じで原因を区別できない点、または catch が受け取った値を見ていないので記録が残らない点に触れていれば満たす","ground":"try の範囲に applyTheme の呼び出しまで入っていて同じ catch が受ける点に触れていれば満たす","core_reject":["1つ目が dark と 50 のまま返ると読んでいる","applyTheme が投げた例外は外へ出ると読んでいる","2つ目だけが既定値になると読んでいる"]}'::jsonb,
  'try { … } catch (e) { … } は、try の中で起きた例外を catch が受け止めます。try に入っている行のうち、どれが失敗したのかは catch 側からは分かりません。
...
values (
  ...
  4,
  '意図',
  'const REQUIRED = ["id", "amount", "placedAt"];
...
戻り値の形も揃えてあります。ok が true のときだけ変換済みの order を返すので、呼ぶ側は「ok なら中身は揃っている」と信じて進められます。例外ではなく戻り値にしてあるので、呼ぶ側が try で囲む必要もありません。',
  '[{"match":["problems","すべて","全部"]},{"match":["一度に","まとめて","1回で"]},{"match":["push","最後に","途中で切り上げ"]},{"match":["ok","order","例外ではな","try で囲"]}]'::jsonb,
  '{"core":"見つかった問題をすべて集めてから一度に返すという意図を指していれば満たす","depth":"ok が true のときだけ変換済みの order を返すので呼ぶ側が中身を信じられる点、または例外ではなく戻り値なので呼ぶ側が try で囲まなくてよい点に触れていれば満たす","ground":"problems に push していき最後にまとめて返している点に触れていれば満たす","core_reject":["処理を速くするために最後まで進めていると読んでいる","最初に見つかった問題だけを返していると読んでいる","問題があっても order を返していると読んでいる"]}'::jsonb,
  '"名前" in obj は、そのプロパティが obj にあるかどうかを真偽で返します。

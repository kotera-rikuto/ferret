...
| `scenario` | TEXT | **【2026-09-11 に追加】** 場面（どういう状況でこのコードを読むことになったか）。**2〜3行・120字以内**の CHECK 制約。問題を開いた直後に、暗い背景の上のカードとして1回だけ出す（閉じると消える・再訪のたびに出る）。**表示専用で採点プロンプトには渡さない。NULL 可** |
